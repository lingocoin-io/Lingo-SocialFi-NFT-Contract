...
describe("Lingo NFT Tests", async () => {
    ...
    describe("Withdraw Function", function () {
        ...
        it("should allow only the owner to withdraw", async function () {
            ...
            await expect(lingoNFT.connect(user1).withdraw()).to.be.revertedWithCustomError(lingoNFT, "OwnableUnauthorizedAccount");
        });
        ...
        it("should revert if there is no Ether to withdraw", async function () {
            await expect(lingoNFT.connect(owner).withdraw({ gasLimit: 30000000 })).to.be.revertedWithCustomError(lingoNFT, "CannotBeZero");
        });
    });
});
